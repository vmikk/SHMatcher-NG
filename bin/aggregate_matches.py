...
De novo SH naming
- Original pipeline: singletons were labeled sequentially as "s1", "s2", ... in `parse_matches_sh.pl`
- Current: this script adds threshold label to the de novo SH code (in "SH code (<TH>)" columns):
  - New clusters (multiple queries, no refs): `dnSH-<TH>-C{n}` (e.g., `dnSH-3.0-C1`)
  - Singletons (single query, no refs):       `dnSH-<TH>-S{n}` (e.g., `dnSH-3.0-S1`)
  `<TH>` is the dissimilarity threshold label in {3.0, 2.5, 2.0, 1.5, 1.0, 0.5}
  Counters `{n}` are per-threshold. For new clusters, all member queries at a given
  threshold share the same code based on a normalized membership signature (sorted member IDs)
